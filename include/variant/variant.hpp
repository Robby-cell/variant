...
namespace detail {
template <std::size_t Count> struct StorageIndex {
  using Type = std::conditional_t<
      Count <= std::numeric_limits<std::uint8_t>::max(), std::uint8_t,
      std::conditional_t<
          Count <= std::numeric_limits<std::uint16_t>::max(), std::uint16_t,
          std::conditional_t<Count <= std::numeric_limits<std::uint32_t>::max(),
                             std::uint32_t, std::uint64_t>>>;
};

template <class T, class... Ts> struct AllTheSame {};

template <class T, class T2, class... Ts> struct AllTheSame<T, T2, Ts...> {
  static constexpr bool Value =
      (std::is_same_v<T, T2> and AllTheSame<T2, Ts...>::Value);
};

template <class T, class T2> struct AllTheSame<T, T2> {
  static constexpr bool Value = (std::is_same_v<T, T2>);
};

template <class T> struct AllTheSame<T> {
...
template <class T, class... Ts> struct IsOneOf<T, T, Ts...> {
  static constexpr bool Value = true;
};

template <class T, class U, class... Ts> struct IsOneOf<T, U, Ts...> {
  static constexpr bool Value = IsOneOf<T, Ts...>::Value;
};

template <class T> struct IsOneOf<T> {
...
private:
  ...
  constexpr static void DummySameReturnTypes(std::index_sequence<Indexes...>) {
    static_assert(
        detail::AllTheSameV<
            ReturnType, decltype(std::declval<Fn>()(
                            std::declval<Self>().template Get<Indexes>()))...>,
        "All return types must be the same");
  }

  template <class ReturnType, class Fn, class Self> struct VisitImplStruct {
  ...
  template <class Fn, class Self> struct VisitImplStruct<void, Fn, Self> {
    static decltype(auto) DoVisit(Fn &&fn, Self &&self) {
  ...
  template <typename T, typename... Pack> struct TypeIndex;
  ...
  template <std::size_t Index> void DoDestroyAlternative() {
    if (type_index_ == Index) {
      reinterpret_cast<TypeAtIndex<Index> *>(&data_)->~TypeAtIndex<Index>();
    }
  }
  ...
  constexpr bool HasInvalidIndex() const { return type_index_ == InvalidIndex; }

  void throw_if_invalid_index() const {
    if (HasInvalidIndex()) {
...
public:
  ...
  template <typename Fn> decltype(auto) Visit(Fn &&fn) & {
    throw_if_invalid_index();
    return DoVisitImpl(std::forward<Fn>(fn), *this);
  }

  template <typename Fn> decltype(auto) Visit(Fn &&fn) const & {
    throw_if_invalid_index();
    return DoVisitImpl(std::forward<Fn>(fn), *this);
  }
